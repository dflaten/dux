...
impl App {
            ...
            match action {
                ...
                SeqAction::Mouse(mouse_ev, raw) => {
                    ...
                    if is_scroll {
                        ...
                        if self.open_terminal_link_from_mouse(mouse_ev) {
                            continue;
                        }
    ...
    fn open_terminal_link_from_mouse(&mut self, mouse_ev: MouseEvent) -> bool {
        if !matches!(mouse_ev.kind, MouseEventKind::Down(MouseButton::Left))
            || !is_terminal_link_click_modifier(mouse_ev.modifiers)
        {
            return false;
        }

        let Some(url) = self.terminal_url_at_screen_pos(mouse_ev.column, mouse_ev.row) else {
            return false;
        };
        ...
        match crate::browser::open_url(&url) {
            ...
            Err(err) => self.set_error(format!("Could not open {url}: {err:#}")),
        }
        true
    }
...
"##,
    ...
    #[test]
    fn interactive_agent_hides_hardware_cursor_without_recent_local_input() {
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
    ...
    #[test]
    fn interactive_agent_strips_reverse_video_from_pty_cursor_cell() {
        use ratatui::Terminal;
        use ratatui::backend::TestBackend;
        ...
        let args = vec![
            "-c".to_string(),
            "printf '\\033[5;9H\\033[7mX\\033[5;9H'; sleep 30".to_string(),
        ];
        let client = PtyClient::spawn("/bin/sh", &args, std::path::Path::new("."), 24, 80, 100)
            .expect("spawn pty");
        app.providers.insert(session_id, client);
        ...
        let cursor_cell = terminal
            .backend()
            .buffer()
            .cell((term_area.x + 8, term_area.y + 4))
            .expect("cursor cell should be inside the rendered buffer");
    ...
    #[test]
    fn iterm_colon_sgr_mouse_selection_pastes_to_terminal() {
        let mut app = test_app(default_bindings());
        install_mouse_layout(&mut app);
        let session_id = app.sessions[0].id.clone();
        let worktree_path = app.sessions[0].worktree_path.clone();
        ...
        // terminal area starts at screen column 21, row 1.
        app.process_raw_input_bytes(b"\x1b[<0:22:2M")
            .expect("mouse down");
        app.process_raw_input_bytes(b"\x1b[<32:30:2M")
            .expect("mouse drag");
