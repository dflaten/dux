...
- Do all code changes on a feature branch. Do not make implementation changes directly on `main`.
- Open a pull request for every completed change set. Keep the PR focused on one coherent change.
- Before starting work, check the current branch and create/switch to a feature branch when needed.
- Before handing work back, report the branch name, files changed, and verification commands run.

## Dev Version

- `DEV_VERSION` is the source of truth for non-release build labels such as `dev-0.0.3`.
- The dev version represents the next development build being worked on, not the last released or last merged build.
- When starting a new change, verify `DEV_VERSION` is already the next expected dev build. If it is stale, update it as part of the change.
- After a PR is merged, `.github/workflows/dev-version.yml` bumps `DEV_VERSION` on the base branch so the next change starts from the next dev build.
- Release builds still use the Cargo package version from `Cargo.toml`; do not try to put `dev-*` values in `Cargo.toml`.
